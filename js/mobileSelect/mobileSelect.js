 ...
 * http://www.jq22.com/jquery-info14679
 */
...
(function() {
	...
	MobileSelect.prototype = {
		...
		init: function(config){
			...
			if(!_this.trigger){/*没有按钮元素报错不支持*/
//				_this.trigger = document.createElement('span');
//				_this.trigger.setAttribute('id','MobileSelectBtn');
//				_this.trigger.innerText = 'MobileSelectBtn';
//				_this.trigger.style.display= 'none';
//				document.body.append(_this.trigger);
...
//				return false;
			...
			//按钮监听
			_this.cancelBtn.addEventListener('click',function(){//取消按钮  - 点击取消按钮  hide
				_this.mobileSelect.classList.remove('mobileSelect-show');
		   });
		   
		    _this.ensureBtn.addEventListener('click',function(){//确认按钮   - 点击确认按钮 hide + callback
				_this.mobileSelect.classList.remove('mobileSelect-show');
				var tempValue ='';
		    ...
		    if(!!_this.trigger){
		    	...
		    	_this.trigger.addEventListener('click',function(){ //触发按钮(trigger) - 点击触发按钮 show
		    		_this.mobileSelect.classList.add('mobileSelect-show');
		    	});
		    }
		    _this.grayLayer.addEventListener('click',function(){ //遮罩层  - 点击遮罩层 hide
		    	_this.mobileSelect.classList.remove('mobileSelect-show');
		    });
		    _this.popUp.addEventListener('click',function(){ //内容容器
		    	event.stopPropagation(); 
		    });
			_this.fixRowStyle(); //修正列数
			
			//添加 _this.hide() _this.show()
//			_this.show = function () {
//				_this.mobileSelect.classList.add('mobileSelect-show');
//			};
//			_this.hide= function () {
...
//			};
		...
		renderWheels: function(wheelsData, cancelBtnText, ensureBtnText){
			...
			//列
				...
				if(!!_this.createLi && _this.createLi instanceof Function){
					for(var j=0; j<wheelsData[i].data.length; j++){ 
						//行
						tempHTML += _this.createLi(wheelsData[i].data[j],j,wheelsData[i],i);
					}
